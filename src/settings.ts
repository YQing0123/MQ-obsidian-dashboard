...
export interface AiQaWebSearchSettings { enabled: boolean; displayName: string; providerId: string; baseUrl: string; protocol: AiQaProtocol; modelId: string; apiKeyKeychainId?: string; }
...
export interface AiQaSettings { providers: AiQaProvider[]; defaultModel?: { providerId: string; modelId: string }; webModel?: { providerId: string; modelId: string }; webSearch: AiQaWebSearchSettings; deepResearchRounds: number; sessionFolder: string; mcpServers: AiQaMcpServer[]; }
...
export const DEFAULT_SETTINGS: DashboardSettings = {
	...
	aiQa: { providers: [], webSearch: { enabled: false, displayName: '联网搜索', providerId: 'web-search', baseUrl: '', protocol: 'openai-compatible', modelId: '' }, deepResearchRounds: 3, sessionFolder: 'AI问答', mcpServers: [{ id: 'sag-knowledge', displayName: 'SAG 知识库', transport: 'streamable-http', url: 'http://localhost:8000/mcp/', enabled: true, readOnlyByDefault: true, authKeychainId: 'mq-aiqa-mcp-sag-knowledge' }] },
...
export class DashboardSettingTab extends PluginSettingTab {
	...
	private renderAiQaSettings(containerEl: HTMLElement): void {
		...
		const web = config.webSearch;
		new Setting(containerEl).setName('联网搜索模型').setHeading();
		new Setting(containerEl).setName('启用联网搜索').setDesc('联网搜索使用独立的 OpenAI 接口配置，不会改变当前回答模型。').addToggle((toggle) => toggle.setValue(web.enabled).onChange(async (value) => { web.enabled = value; await this.plugin.saveSettings(); this.display(); }));
		new Setting(containerEl).setName('搜索模型显示名称').addText((text) => text.setValue(web.displayName).setPlaceholder('联网搜索').onChange(async (value) => { web.displayName = value.trim() || '联网搜索'; await this.plugin.saveSettings(); }));
		new Setting(containerEl).setName('搜索提供方 ID').addText((text) => text.setValue(web.providerId).setPlaceholder('web-search').onChange(async (value) => { web.providerId = value.trim() || 'web-search'; await this.plugin.saveSettings(); }));
		new Setting(containerEl).setName('搜索 API 地址').addText((text) => text.setValue(web.baseUrl).setPlaceholder('https://api.example.com/v1').onChange(async (value) => { web.baseUrl = normalizeOpenAiBaseUrl(value); await this.plugin.saveSettings(); }));
		new Setting(containerEl).setName('搜索 API 协议').addDropdown((dropdown) => dropdown.addOption('openai-compatible', 'OpenAI Compatible').addOption('openai-responses', 'OpenAI Responses').setValue(web.protocol).onChange(async (value) => { web.protocol = value as AiQaProtocol; await this.plugin.saveSettings(); }));
		new Setting(containerEl).setName('搜索 API 密钥').addText((text) => { const saved = web.apiKeyKeychainId ? this.app.secretStorage?.getSecret(web.apiKeyKeychainId) : ''; text.setPlaceholder('留空保持现有密钥').setValue(saved ? '********' : ''); text.inputEl.type = 'password'; text.onChange(async (value) => { if (value && value !== '********') { web.apiKeyKeychainId ||= `mq-aiqa-web-${crypto.randomUUID()}`; if (!this.app.secretStorage) { new Notice('当前 Obsidian 不支持安全密钥存储，未保存 API Key'); return; } this.app.secretStorage.setSecret(web.apiKeyKeychainId, value); await this.plugin.saveSettings(); } }); });
		new Setting(containerEl).setName('搜索模型 ID').addText((text) => text.setValue(web.modelId).setPlaceholder('例如 gpt-4o-mini').onChange(async (value) => { web.modelId = value.trim(); await this.plugin.saveSettings(); }));
		new Setting(containerEl).setName('测试联网模型').addButton((button) => button.setButtonText('测试连接').onClick(async () => { try { if (!web.baseUrl || !web.modelId) throw new Error('请先填写搜索 API 地址和模型 ID'); const key = web.apiKeyKeychainId ? this.app.secretStorage?.getSecret(web.apiKeyKeychainId) : ''; await requestUrl({ url: `${normalizeOpenAiBaseUrl(web.baseUrl)}/models`, headers: key ? { Authorization: `Bearer ${key}` } : {} }); new Notice('联网搜索模型连接成功'); } catch (error) { new Notice(`联网模型连接失败：${error instanceof Error ? error.message : String(error)}`); } }));
