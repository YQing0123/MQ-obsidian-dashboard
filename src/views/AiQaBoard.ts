...
export class AiQaBoard {
  ...
  private mount(root: HTMLElement): void {
    ...
    style.textContent = `
      ...
      @media(max-width:760px){.mq-ai-qa-board{grid-template-columns:1fr;height:auto;min-height:650px}.mq-ai-qa-board .qa-side{max-height:180px;border-right:0;border-bottom:1px solid var(--background-modifier-border)}.mq-ai-qa-board .qa-history{display:flex;gap:4px;overflow-x:auto}.mq-ai-qa-board .qa-history-label{display:none}.mq-ai-qa-board .qa-history-item{min-width:145px}.mq-ai-qa-board .qa-header{padding:0 13px}.mq-ai-qa-board .qa-transcript,.mq-ai-qa-board .qa-composer{padding-left:13px;padding-right:13px}.mq-ai-qa-board .qa-composer-bar{align-items:flex-end}.mq-ai-qa-board .qa-select{max-width:145px}}
    `;
  ...
  private async submit(): Promise<void> {
    ...
    try {
      ...
      this.messages.push(user, assistant); this.active.model = selected.ref; this.active.title = this.active.title === '新问答' ? query.replace(/(?:^|\s)@[^\s@]+/gu, ' ').replace(/\s+/g, ' ').trim().slice(0, 42) || '新问答' : this.active.title; this.input!.value = ''; this.syncSessionControls(); this.renderHistory(); this.renderMessages(); this.schedulePersist();
