...
export interface AiQaSettings { providers: AiQaProvider[]; defaultModel?: { providerId: string; modelId: string }; deepResearchRounds: number; sessionFolder: string; mcpServers: AiQaMcpServer[]; collapseBannerOnOpen?: boolean; }
...
export class DashboardSettingTab extends PluginSettingTab {
	...
	private renderAiQaSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl).setName('进入 AI 问答时收起横幅').setDesc('切换到 AI 问答菜单时自动折叠顶部横幅，为问答区域留出更多高度。').addToggle((toggle) => toggle.setValue(config.collapseBannerOnOpen === true).onChange(async (value) => { config.collapseBannerOnOpen = value; await this.plugin.saveSettings(); }));
