...
export class AiQaBoard {
  ...
  private mount(root: HTMLElement): void {
    ...
    this.renderModelOptions(); this.renderReasoningOptions(); this.renderAttachments();
  }
  ...
  private async newSession(): Promise<void> {
    this.abort?.abort(); const now = Date.now(); this.active = { id: crypto.randomUUID(), title: '新问答', createdAt: now, updatedAt: now, archived: false, webEnabled: false, mode: 'normal' }; this.messages = []; await this.persist(); this.syncSessionControls(); this.renderHistory(); this.renderMessages(); this.input?.focus();
  }
  private async selectSession(session: AiQaSession): Promise<void> { const saved = await this.store.read(session.id); if (!saved) return; this.abort?.abort(); this.active = saved.session; this.messages = saved.messages ?? []; this.syncSessionControls(); this.renderHistory(); this.renderMessages(); }
  private syncSessionControls(): void { if (!this.active) return; if (this.titleEl) this.titleEl.textContent = this.active.title || 'AI 问答'; if (this.modeSelect) this.modeSelect.value = this.active.mode || 'normal'; if (this.active.model && this.modelSelect) this.modelSelect.value = modelKey(this.active.model); this.renderReasoningOptions(); if (this.webToggle) this.webToggle.checked = Boolean(this.active.webEnabled); this.renderAttachments(); }
  ...
  private async submit(): Promise<void> {
    ...
    try {
      ...
      const content = query + textAttachments + evidence; const user: AiQaMessage = { id: crypto.randomUUID(), sessionId: this.active.id, role: 'user', content, createdAt: Date.now(), delivery: 'complete', attachments }; const steps: AiQaStep[] = Array.from({ length: rounds }, (_, index) => ({ id: crypto.randomUUID(), kind: index === rounds - 1 ? 'retrieval' : 'thinking', label: rounds > 1 ? `研究轮次 ${index + 1}/${rounds} · ${index === rounds - 1 ? '汇总本地证据' : '分析检索方向'}` : hits.length ? `本地知识库检索命中 ${hits.length} 篇` : '本地知识库未命中', status: 'done', count: index === rounds - 1 ? hits.length : undefined })); const assistant: AiQaMessage = { id: crypto.randomUUID(), sessionId: this.active.id, role: 'assistant', content: '', createdAt: Date.now(), delivery: 'streaming', steps: [...steps, { id: crypto.randomUUID(), kind: 'answer', label: '正在整理回答', status: 'active' }], citations };
