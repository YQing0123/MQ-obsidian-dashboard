...
export interface AiQaSettings { providers: AiQaProvider[]; defaultModel?: { providerId: string; modelId: string }; deepResearchRounds: number; sessionFolder: string; mcpServers: AiQaMcpServer[]; }
...
export const DEFAULT_SETTINGS: DashboardSettings = {
	...
	aiQa: { providers: [], deepResearchRounds: 3, sessionFolder: 'AI问答', mcpServers: [{ id: 'sag-knowledge', displayName: 'SAG 知识库', transport: 'streamable-http', url: 'http://localhost:8000/mcp/', enabled: true, readOnlyByDefault: true, authKeychainId: 'mq-aiqa-mcp-sag-knowledge' }, { id: 'firecrawl', displayName: 'Firecrawl 联网搜索', transport: 'streamable-http', url: 'https://mcp.firecrawl.dev/v2/mcp-oauth', enabled: false, readOnlyByDefault: true, authKeychainId: 'mq-aiqa-mcp-firecrawl' }] },
...
export class DashboardSettingTab extends PluginSettingTab {
	...
	private renderAiQaSettings(containerEl: HTMLElement): void {
		...
		for (const server of config.mcpServers) {
			...
			const block = containerEl.createEl('details', { cls: 'mq-ai-qa-mcp-settings' }); block.open = server.id === 'firecrawl' || server.id === 'sag-knowledge';
			const summary = block.createEl('summary'); summary.createSpan({ cls: 'mq-ai-qa-mcp-summary-title', text: server.displayName || 'MCP 服务' }); summary.createSpan({ cls: 'mq-ai-qa-mcp-summary-meta', text: `${server.transport} · ${server.enabled ? '已启用' : '已停用'}` });
			new Setting(block).setName('显示名称').addText((text) => text.setValue(server.displayName).onChange(async (value) => { server.displayName = value.trim() || 'MCP 服务'; summary.querySelector('.mq-ai-qa-mcp-summary-title')?.setText(server.displayName); await this.plugin.saveSettings(); }));
			new Setting(block).setName('服务地址').setDesc('Streamable HTTP MCP 端点。').addText((text) => text.setValue(server.url || '').setPlaceholder('https://example.com/mcp').onChange(async (value) => { server.url = value.trim(); await this.plugin.saveSettings(); }));
			new Setting(block).setName('访问令牌').setDesc('令牌仅保存到 Obsidian 安全存储。').addText((text) => { text.setPlaceholder('留空保持现有令牌').setValue(this.app.secretStorage?.getSecret(authKey) ? '********' : ''); text.inputEl.type = 'password'; text.onChange(async (value) => { if (value && value !== '********' && this.app.secretStorage) { this.app.secretStorage.setSecret(authKey, value); await this.plugin.saveSettings(); } }); });
			new Setting(block).setName('启用服务').addToggle((toggle) => toggle.setValue(server.enabled).onChange(async (value) => { server.enabled = value; summary.querySelector('.mq-ai-qa-mcp-summary-meta')?.setText(`${server.transport} · ${value ? '已启用' : '已停用'}`); await this.plugin.saveSettings(); }));
			new Setting(block).addButton((button) => button.setButtonText('删除服务').setWarning().onClick(async () => { config.mcpServers = config.mcpServers.filter((item) => item.id !== server.id); await this.plugin.saveSettings(); this.display(); }));
		}
	}
