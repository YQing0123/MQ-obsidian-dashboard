...
export class BannerEditModal extends Modal {
	...
	private renderBody(): void {
		...
		const accent = appearance.createDiv({ cls: 'ad-banner-modal__row' }); accent.createSpan({ text: '强调色' }); const color = accent.createEl('input', { attr: { type: 'color' } }); color.value = this.draft.accent || '#bff038'; color.addEventListener('input', () => { this.draft.accent = color.value; });
		const details = this.form.createEl('label', { cls: 'ad-banner-modal__check' }); const cb = details.createEl('input', { attr: { type: 'checkbox' } }); cb.checked = this.draft.showDetails !== false; cb.addEventListener('change', () => { this.draft.showDetails = cb.checked; }); details.createSpan({ text: '显示详细条带、热力图和进度条' });
	}
	...
	private addCheck(parent: HTMLElement, label: string, key: 'showLeft' | 'showCenter' | 'showRight'): void {
		const check = parent.createEl('label', { cls: 'ad-banner-modal__check' }); const input = check.createEl('input', { attr: { type: 'checkbox' } }); input.checked = this.draft[key] !== false; input.addEventListener('change', () => { this.draft[key] = input.checked; }); check.createSpan({ text: label });
	}

	private addRange(parent: HTMLElement, label: string, key: 'blur' | 'darkness', value: number, min: number, max: number): void {
		const row = parent.createDiv({ cls: 'ad-banner-modal__range' }); row.createSpan({ text: label }); const input = row.createEl('input', { attr: { type: 'range', min: String(min), max: String(max), value: String(value) } }); const valueEl = row.createSpan({ text: String(value) }); input.addEventListener('input', () => { const n = Number(input.value); valueEl.textContent = String(n); this.draft[key] = n; });
	}
